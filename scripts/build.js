...
function copy(tmpPath) {
  console.log(`🔨 Copy`)
  return promisify(fs.copyFile)('LICENSE.md', `${tmpPath}/LICENSE.md`)
}
...
async function build(tmpPath) {
  ...
  const pkgFullName = `rxcontrol${pkgName === 'core' ? '' : `-${pkgName}`}`
...
(async () => {
  const tmpPath = path.join(process.cwd(), `tmp/build/${shortid()}`)
